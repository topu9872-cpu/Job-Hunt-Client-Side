...
import { getApplicationByApply, getJobById } from "@/app/api/Server/Server";
...
const ApplyPage = async ({ params }) => {
    ...
    return (
      <div className="min-h-[70vh] flex items-center justify-center px-4 bg-gray-50/50">
        <div className="max-w-md w-full bg-white border border-gray-200 rounded-2xl p-8 text-center shadow-sm">
          ...
          <h3 className="text-xl font-bold text-gray-900 mb-2">Application Restricted</h3>
          <p className="text-gray-600 text-sm leading-relaxed">
            Only verified job seekers can submit applications. If you are registered as a recruiter, please switch accounts.
          </p>
        </div>
      </div>
    );
  ...
  const plan = {
    name: "Free",
    maxApplicationsPerMonth: 3, // Cleaned up the variable typo safely
  };

  // Check if they have remaining monthly applications
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <p className="text-xs font-bold uppercase tracking-wider text-gray-400 mb-1">
                Your Current Plan: <span className="text-blue-600">{plan.name}</span>
              </p>
            ...
            <div className="text-left sm:text-right">
              <span className="text-3xl font-black ">{applications.length}</span>
              <span className="text-gray-400 font-medium text-lg"> / {plan.maxApplicationsPerMonth}</span>
              <p className="text-xs text-gray-500 mt-0.5">Submissions this month</p>
            </div>
          </div>
        ...
        {hasRemainingApplications ? (
          ...
          <div className="border border-amber-200 rounded-2xl p-8 text-center shadow-sm">
            ...
            <p className="text-gray-600 text-sm max-w-md mx-auto mb-6 leading-relaxed">
              You have successfully used all {plan.maxApplicationsPerMonth} of your free applications for this cycle. Ready to apply to more positions?
            </p>
            <Link 
